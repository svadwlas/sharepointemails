...
namespace SharePointEmails.Core.Substitutions
{
    class ComplexSubstitution:ISubstitution
    {
        ...
        public string Process(string text, ISubstitutionContext context, ProcessMode mode)
        {
            ...
            foreach (Match m in Regex.Matches(res, @"\{\$(.+)\}"))
            {
                try
                {
                    ...
                    {
                        case "changes": value = GetGhanges(context,mode); break;
        ...
        private string GetGhanges(ISubstitutionContext context, ProcessMode mode)
        {
            var sb = new StringBuilder();
            if (mode != ProcessMode.Test)
            {
            ...
            else
            {
                sb.Append("<table class='changes-table'>");
                sb.Append(string.Format("<tr><th>Field Name</th><th>Field Old Value</th><th>Field New Value</th></tr>"));
                foreach (var item in new string[] { "field1", "field2", "field3" })
                {
                    sb.Append(string.Format(@"<tr><td>{0}</td><td>{1}</td><td>{2}</td></tr>", item, item + "old value", item + "new value"));
                }
                sb.Append("</table>");
            }
